...
<div class="row">
    <div class="col-md-12">
        <form action="{{ route('markupdate') }}" method="post">
            @csrf
        ...
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6">
                        <h5 class="text-success"><strong>UPDATE MARK</strong></h5>
                    </div>
                    <div class="col-md-6">
                        <a href="{{ route('mark.index') }}" class="btn btn-success float-right"><strong>Add new</strong></a>
                    </div>
                </div>
            </div>
            <div class="card-body">
...
@section('script')
    ...
    <script>
        function loadData(){
            ...
            $.ajax({
                    url: "{{ url('/get/all/marks/') }}"+"/"+exam_id+"/"+class_id+"/"+section_id+"/"+subject_id,
                    type: "GET",
                    datatype: "json",
                    success: function(data) {
                        ...
                        $.each(data, function(key, value) {
                            html += '<tr>';
                            html += '<td><input type="hidden" name="student_id[]" value="'+value.student.id+'">'+(i++)+'</td>'
                            html += '<td>'+value.student.name+'</td>'
                            html += '<td>'+value.student.username+'</td>'
                            html += '<td>'+value.student.roll_no+'</td>'
                            html += '<td><input type="number" class="form-control" name="marks[]" value="'+value.marks+'"></td>'
                            html += '</tr>';
                        });
                        $('.tbody').append(html);
                    }
